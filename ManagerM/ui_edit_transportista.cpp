...
#include "ui_ui_edit_transportista.h"

ui_edit_transportista::ui_edit_transportista(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ui_edit_transportista)
{
    ui->setupUi(this);
...
ui_edit_transportista::~ui_edit_transportista()
{
    delete ui;
}

void ui_edit_transportista::update_data(QString ruc)
{
    currentRUC = ruc;
    ...
    object_e_transportista o_t;
    o_t.mf_load(ruc);
    ui->lineEdit_ruc->setText(o_t.mf_get_pk_ruc());

}

void ui_edit_transportista::setTableView(QTableView *t)
{
    table = t;
}

void ui_edit_transportista::on_pushButton_guardar_clicked()
{
    ...
    if(rpta == 0)
    {
        ...
        object_e_transportista o_t;
        o_t.mf_set_pk_ruc(ui->lineEdit_ruc->text());


        str_query = "UPDATE e_transportista ";
        str_query += QString("SET pk_ruc = '")+o_t.mf_get_pk_ruc()+QString("'");
        str_query += QString(" WHERE pk_ruc='")+currentRUC+QString("'");

        query.prepare(str_query);
        if(query.exec()){
            QSqlQueryModel* model = new QSqlQueryModel;
            model->setQuery("SELECT * FROM getTransportistas");
...
void ui_edit_transportista::on_pushButton_link_clicked()
{
...
void ui_edit_transportista::on_pushButton_cancelar_clicked()
{
